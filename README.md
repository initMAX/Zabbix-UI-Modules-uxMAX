...
Paging and mass actions mirrored above long lists, so there is no scroll to the bottom and back. Discovery rules support **Execute now** from either action bar.
...
A switch in the user menu. System is the default for users who have not made a choice yet; it follows the operating system's own setting and keeps following it, which Zabbix on its own cannot do.
